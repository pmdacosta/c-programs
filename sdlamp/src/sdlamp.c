...
feed_audio_device_callback(void *userdata, Uint8 *output_stream, int len)
{
    ...
    int num_converted_bytes = SDL_AudioStreamGet(input_stream, output_stream, len);
...
static SDL_bool
open_new_audio_file(char *fname)
{
...
handle_events(WinAmpSkin *skin)
{
    ...
    while (SDL_PollEvent(&e))
    {
        ...
        case SDL_DROPFILE:
        {
            char *ptr = SDL_strrchr(e.drop.file, '.');
            if (ptr && ((SDL_strcasecmp(ptr, ".wsz") == 0) || (SDL_strcasecmp(ptr, ".wsz") == 0)))
            {
